...
def main():
    ...
    # with open('~.txt','r') as f:
      # f.readlines()[0].split().strip()
      # ['지선님은 혁훈님과 동료입니다.','그래서 대회 나갔습니다.']
    print(args.start_count,'--',args.start_count+ args.count)
    strings = []
    import pickle
    if args.hyukhun:
        # with open('./refine_kosen.pkl','rb') as f:
        #     strings = pickle.load(f)[args.start_count:args.start_count+args.count]
        with open('./test_word.txt','r',encoding='utf-8') as f:
            strings = f.read().split('\n')
            strings = [i.strip() for i in strings]
    print(len(strings))
    strings = strings[args.start_count:args.start_count+args.count]        
